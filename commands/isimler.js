...
if (!isimler) return message.channel.send(new MessageEmbed().setAuthor(message.author.tag,message.author.displayAvatarURL({dynamic:true})).setColor('RANDOM').setDescription(`Bu Kullanıcının Toplam "0" Kayıtlı İsmi Bulundu`)).then(message.react('❤')).then(lrows => lrows.delete({timeout:10000}))
