...
if (member.user.bot) return message.channel.send(embed.setDescription(`${message.author} Botların isimini değiştiremezsin!`)).then(lrows => lrows.delete({timeout : 5000})).then(message.react(emoji.redemoj))
if (member.id === message.author.id) return message.channel.send(embed.setDescription(`${message.author} Kendi ismini değiştiremezsin!`)).then(lrows => lrows.delete({timeout : 5000})).then(message.react(emoji.redemoj))
...
if (member.user.username.includes(taglı)) {
    ...
    .setColor('BLACK')
    .setDescription(`${member} adlı kullanıcın ismi başarıyla \`${taglı} ${isim} | ${yaş}\` olarak değiştirildi ${emoji.lrows} `)
    ...
    .setAuthor(message.guild.name, message.guild.iconURL({ dynamic: true }))
    .setColor('BLACK')
    .setDescription(`${member} adlı kullanıcın ismi başarıyla \`${tagsız} ${isim} | ${yaş}\` olarak değiştirildi ${emoji.lrows} `)
    .setFooter(config.bots.footer)
    message.channel.send(lrows)
    message.react(emoji.onayemoji)
