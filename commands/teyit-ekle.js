...
if (member.user.bot) return message.channel.send(embed.setDescription(`${message.author} Botların kayıt datası olmaz !`)).then(lrows => lrows.delete({timeout : 5000})).then(message.react(emoji.redemoj))
if (member.id === message.author.id) return message.channel.send(embed.setDescription(`${message.author} Kendinine kayıt ekleyemezsin!`)).then(lrows => lrows.delete({timeout : 5000})).then(message.react(emoji.redemoj))
...
if(!args[1]) {
    ...
    .teyitekle @Lrows/ID erkek <miktar>
    .teyitekle @Lrows/ID kadın <miktar>`)
    .setFooter(config.bots.footer)
    .setTimestamp()
    ).then(lrows => lrows.delete({ timeout : 10000 })).then(message.react(emoji.redemoj))
}

if(args[1] === 'erkek') {
let miktar = args[2]
if(!miktar) return message.channel.send(embed.setDescription(`eklemek istediğinz kadar sayı girin!`)).then(lrows => lrows.delete({timeout : 7000})).then(message.react(emoji.redemoj)) 
    ...
    .then(lrows => {
    setTimeout(() => {
    lrows.edit(embed.setTimestamp().setDescription(`${member} adlı kullanıcının yeni kayıt datası:
    Erkek Kayıt \`${erkek}\`
    Toplam Kayıt\`${toplam}\`
...
if(args[1] === 'kadın') {
    ...
    exports.help = {
        name : 'kayıt-ekle',
        help: "kayıt-ekle [lrows/ID] [erkek/kız] [miktar]",
        cooldown: 0
     }
