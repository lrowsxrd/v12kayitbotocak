...
if (member.id === message.author.id) return message.channel.send(embed.setDescription(`${message.author} Kendi kayıt datanı silemezsin!`)).then(lrows => lrows.delete({timeout : 5000})).then(message.react(emoji.redemoj))
...
if(!args[1]) {
    ...
    .teyitsil @Lrows/ID erkek <miktar>
    .teyitsil @Lrows/ID kadın <miktar>`)
...
if(args[1] === 'erkek') {
let miktar = args[2]
if(!miktar) return message.channel.send(embed.setDescription(`silmek istediğinz kadar sayı girin!`)).then(lrows => lrows.delete({timeout : 7000})).then(message.react(emoji.redemoj)) 
...
if(args[1] === 'kadın') {
    ...
    exports.help = {
        name : 'kayıt-sil',
        help: "kayıt-sil [lrows/ID] [erkek/kız] [miktar]",
        cooldown: 0
     }
