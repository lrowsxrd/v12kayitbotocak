...
module.exports = {
  ...
  if (cmd) {
    ...
    if (client.cooldowns.has(`${command}_${message.author.id}`)) {
        ...
        return message.channel.send(embed.setDescription(`Bu komudu tekrardan kullanabilmek için **${kalan} saniye** beklemeniz gerekmektedir.`)).then(lrows => lrows.delete({ timeout : 5000 })).then(message.react(emoji.redemoj))
    };
